...
def test_signup_and_unregister():
    ...
    # Unregister
    response_unreg = client.post(f"/activities/{activity}/unregister?email={test_email}")
    assert response_unreg.status_code == 200
    assert f"Removed {test_email}" in response_unreg.json()["message"]
    # Unregister again (should fail)
    response_unreg2 = client.post(f"/activities/{activity}/unregister?email={test_email}")
